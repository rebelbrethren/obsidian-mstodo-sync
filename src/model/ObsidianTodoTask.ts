...
import {type MsTodoSyncSettings} from 'src/gui/msTodoSyncSettingTab';
...
export class ObsidianTodoTask implements TodoTask {
  ...
  private readonly settings: MsTodoSyncSettings;
  ...
  public async cacheTaskId(id: string): Promise<void> {
    ...
    const index = `${Math.random().toString(20).slice(2, 6)}${this.settings.taskIdIndex
      .toString()
      .padStart(5, '0')}`;
